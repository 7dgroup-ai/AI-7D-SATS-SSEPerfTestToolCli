...
sse_perfTestTool.py    # Entry point: CLI argument parsing, thread orchestration
    ├── providers.py       # QueryProvider, ApiKeyProvider (thread-safe cycling)
    ├── tester.py          # SSETester class: HTTP/SSE handling, metrics calculation
    ├── test_runner.py     # run_test_thread(), aggregate_stats() for real-time stats
    └── report_generator.py # HTML report generation with Chart.js visualizations
```
